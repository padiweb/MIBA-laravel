...
<div class="miba-card" style="max-width:440px">
  <div class="miba-card-header">
    <div class="miba-card-title"><i class="fa fa-key"></i> Reset Password — {{ $user->user_full_name }}</div>
  ...
  <div class="miba-card-body">
    <form method="POST" action="{{ route('users.resetPassword',$user->user_id) }}">@csrf
      <div class="miba-form-group"><label class="miba-label">Password Baru <span class="req">*</span></label><input type="password" name="user_password" class="miba-input" required minlength="6"></div>
      ...
      <div style="display:flex;gap:8px">
        ...
        <a href="{{ route('users.index') }}" class="btn-miba btn-ghost-miba">Batal</a>
      </div>
    </form>
  </div>
</div>
